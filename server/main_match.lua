...
if _G["__init__"] then
    ...
    return {
        thread = 16,
        enable_stdout = true,
        logfile = string.format("log/match-%s-%s.log", arg[1], os.date("%Y-%m-%d-%H-%M-%S")),
        loglevel = "DEBUG",
        path = table.concat({
            "./?.lua",
            "./?/init.lua",
...
local function run(node_conf)
    ...
    local services = {
        ...
        },
        {
            unique = true,
            name = "matchmgr",
            file = "match/service_matchmgr.lua",
            threadid = 5,
            websocket = false,
        },
    ...
    local function Start()
        ...
        assert(moon.call("lua", moon.queryservice("matchmgr"), "MatchMgr.Init"))
        ...
        assert(moon.call("lua", moon.queryservice("matchmgr"), "MatchMgr.Start"))
