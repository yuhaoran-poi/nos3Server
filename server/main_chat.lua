...
if _G["__init__"] then
    ...
    return {
        thread = 16,
        enable_stdout = true,
        logfile = string.format("log/chat-%s-%s.log", arg[1], os.date("%Y-%m-%d-%H-%M-%S")),
        loglevel = "DEBUG",
        path = table.concat({
            "./?.lua",
            "./?/init.lua",
...
local function run(node_conf)
    ...
    local services = {
        ...
        },
        {
            unique = true,
            name = "channelmgr",
            file = "chat/service_channelmgr.lua",
            threadid = 5,
            websocket = false,
        },
    ...
    local function Start()
        ...
        assert(moon.call("lua", moon.queryservice("channelmgr"), "Init"))
    ...
    moon.shutdown(function()
        ...
        moon.async(function()
            if server_ok then
                ...
                moon.kill(moon.queryservice("channelmgr"))
            else
                moon.exit(-1)
            end
