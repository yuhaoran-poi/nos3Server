...
if _G["__init__"] then
    ...
    return {
        thread = 16,
        enable_stdout = true,
        logfile = string.format("log/log-%s-%s.log", arg[1], os.date("%Y-%m-%d-%H-%M-%S")),
        loglevel = "DEBUG",
        path = table.concat({
            "./?.lua",
            "./?/init.lua",
...
local function run(node_conf)
    ...
    local function Start()
        ...
        assert(moon.call("lua", moon.queryservice("logmgr"), "LogMgr.Init"))
        ...
        assert(moon.call("lua", moon.queryservice("logmgr"), "LogMgr.Start"))
    ...
    moon.shutdown(function()
        ...
        moon.async(function()
            if server_ok then
                ...
                moon.kill(moon.queryservice("logmgr"))
            else
                moon.exit(-1)
            end
