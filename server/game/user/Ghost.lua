...
function Ghost.LoadGhosts()
    local ghostinfos, db_err = Database.loaduserghosts(context.addr_db_user, context.uid)
    if db_err then
...
function Ghost.UpStar(config_id)
    ...
    local rate_key = "rate" .. (ghost_image_info.star_level + 1)
    ...
    -- 计算升星概率
    local now_rate = rate_cfg + add_rate_cfg.value * ghost_image_info.star_fail_cnt
    local random_rate = math.random(1, 10000)
    if random_rate > now_rate then
        -- 增加升星失败次数
        ghost_image_info.star_fail_cnt = ghost_image_info.star_fail_cnt + 1
        return ErrorCode.UpStarProbFail, change_log
    else
        -- 增加星星
        ghost_image_info.star_level = ghost_image_info.star_level + 1
        ghost_image_info.star_fail_cnt = 0
        return ErrorCode.None, change_log
    end
end
